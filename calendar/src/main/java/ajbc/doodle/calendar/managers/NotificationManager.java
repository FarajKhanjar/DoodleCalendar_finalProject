...
//	private final int NUM_THREADS = 3;
	...
	public void run() throws DaoException {
		...
		while(queue.isEmpty()==false) 
		{
			...
			{
				 pushMsgObj = new PushMessage(notification.getTitle(), notification.getMessage());
				 byte[] result;
					try {
						result = dataManager.getCryptoService().encrypt(dataManager.getObjectMapper().writeValueAsString(pushMsgObj),
								user.getSubscriptionInfo().getPublicKey(),
								user.getSubscriptionInfo().getAuthKey(), 0);
						
						sendPushMessage(user.getSubscriptionInfo().getEndpoint(), result);
						
						System.out.println("Event: " + notification.getEventToNotify().getTitle() +
								           " Title: " + notification.getTitle() +
								           " Message: " + notification.getMessage() +
								           " sent to user " + user.getEmail());
						Thread.sleep(3000);
